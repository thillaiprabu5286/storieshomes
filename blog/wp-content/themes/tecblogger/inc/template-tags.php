...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'tecblogger' ); ?></h2>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( esc_html__( 'Older posts', 'tecblogger' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( esc_html__( 'Newer posts', 'tecblogger' ) ); ?></div>
			<?php endif; ?>
...
function tecblogger_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'tecblogger' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'tecblogger' ),
...
function tecblogger_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && tecblogger_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'tecblogger' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'tecblogger' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'tecblogger' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
?>
	...
	<span class="comments-link">
		<?php comments_popup_link( __( 'Leave a comment', 'candela' ), __( '1 Comment', 'candela' ), __( '% Comments', 'candela' ) ); ?>
	</span>
