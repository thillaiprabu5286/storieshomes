...
function featured_image_in_feed( $content ) {
    ...
    if( is_feed() ) {
        if ( has_post_thumbnail( $post->ID ) ){
            $output = get_the_post_thumbnail( $post->ID, array(825,510), array( 'style' => 'float:right; margin:0 0 10px 10px;','class' => 'smallimg' ) );
            //$output = $output . get_the_post_thumbnail( $post->ID, 'large', array( 'class' => 'largimg' ) );
             //$large_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'medium' ,array( 'class' => 'largimg' ) );
             //$output = $output . $large_image_url;
...
add_filter( 'the_content', 'featured_imagelarge_in_feed' );
function featured_imagelarge_in_feed( $content ) {
    global $post;
    if( is_feed() ) {
        if ( has_post_thumbnail( $post->ID ) ){
            //$output = get_the_post_thumbnail( $post->ID, array(150,150), array( 'style' => 'float:right; margin:0 0 10px 10px;','class' => 'smallimg' ) );
            //$output = $output . get_the_post_thumbnail( $post->ID, 'large', array( 'class' => 'largimg' ) );
           //  $output = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'large' ,array( 'class' => 'largimg' ) );
           $output = get_the_post_thumbnail( $post->ID, 'large', array( 'style' => 'float:right; margin:0 0 10px 10px;','class' => 'largimg' ) );
