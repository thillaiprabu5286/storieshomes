	...
	$.fn.etalage = function(options){
		...
		var o = $.extend({
			...
			source_image_height: 1200,			// The source/zoomed image height (not the frame around it) (value in pixels)
                        disable_zoom : false,  // ADDED BY MANGOEXTENSION - REMOVES ZOOM FUNCTIONALITIES...
...
// NEW ELEMENTS & CACHE
				$container.addClass('etalage').show();
                                ...
                                /* remove elements in case the plugin was already initialized
                                 * added by mangoextensions */
                                $container.children("li.etalage_magnifier, li.etalage_icon, li.etalage_zoom_area, li.etalage_small_thumbs, li.etalage_hint").remove();
                                $container.children("li.etalage_thumb").attr("style","").removeClass("etalage_thumb, etalage_thumb_active");
				...
				// Add magnifier - only if zoom enabled - mangoextensions
                                if(o.disable_zoom == false){
				...
				if(images > 1 || !o.smallthumb_hide_single){
					...
					$.each($thumb_images, function(){
						...
						thumb_id = $t.parents('.etalage_thumb').data('id');
                                                /*added by mangoextension - attribute swatches */
                                                if($t.data('attributes-class')) css_class = ' class="'+ $t.data('attributes-class') +'"';
						$('<li ' + css_class +'><img class="etalage_small_thumb" src="'+src+'" /></li>').data('thumb_id', thumb_id).appendTo($smallthumbs_ul);
					});
...
// RESIZE AND REPOSITION ELEMENTS
				...
				// Icon
				if(o.show_icon){
					css = { top:thumb_outerheight - $icon.outerHeight(true) - parseInt(o.icon_offset, 10), right:parseInt(o.icon_offset, 10) };
					if(o.smallthumbs_position==='right'){
						css.right = smallthumb_outerwidth + smallthumb_margin + parseInt(o.icon_offset, 10);
					}else if(o.smallthumbs_position==='top'){
...
// FUNCTIONS
						...
						next();
                                ...
                                var start_zoom = function(){
                                    ...
                                    if(o.disable_zoom == false){
                                        ...
                                        // do not change opactiy... icon used to trigger fancybox... 
				...
				var stop_zoom = function(){
					...
					$magnifier.stop().fadeOut(o.speed);
                                        ...
                                        // do not animate icon, icon used to trigger fancybox...
				...
				var show = function(number){
					...
					if($number.length && !st_moving){
						...
						/* added by mangoextensions */
						var disable_animation = ( $smallthumbs_ul.width() < $smallthumbs.width() );
						...
						if(active > number){
							...
							if(number < firstid && !disable_animation ){
						...
						// Move forward
							...
							if(number >= lastid && !disable_animation ){
				...
				if(images > 1 || !o.smallthumb_hide_single){
					...
					$smallthumb.click(function(){
						...
						if(!$this.hasClass('etalage_smallthumb_active') && (!st_moving || st_steps)){
							...
							// Slide left / up
							if($this.hasClass('etalage_smallthumb_first') && $this.prev().length && !disable_animation ){
								st_shift_1('left');
							}
							// Shift to end
							else if($this.hasClass('etalage_smallthumb_navtoend') && !disable_animation ){
								st_slide_to_end();
							}

							// Slide right / down
							else if($this.hasClass('etalage_smallthumb_last') && $this.next().length && !disable_animation ){
								st_shift_1('right');
							}
							// Shift to beginning
							else if($this.hasClass('etalage_smallthumb_navtostart') && !disable_animation ){
								st_slide_to_start();
							}
