...
class Mage_SalesRule_Model_Resource_Coupon extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    public function updateSpecificCoupons(Mage_SalesRule_Model_Rule $rule)
    {
        ...
        // Custom field prabu
        if ($rule->dataHasChangedFor('uses_per_mobile')) {
            $updateArray['usage_per_mobile'] = $rule->getUsesPerMobile();
        }

        if ($rule->dataHasChangedFor('uses_per_shopid')) {
            $updateArray['usage_per_shopid'] = $rule->getUsesPerShopid();
        }
