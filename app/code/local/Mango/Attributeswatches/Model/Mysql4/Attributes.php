...
class Mango_Attributeswatches_Model_Mysql4_Attributes extends Mage_Core_Model_Mysql4_Abstract {
    ...
    protected $_attributes_options_array = array();

    /* store attribute data in array for reusing later */
    protected function _getAttributeOptions($_attribute_code) {
        if (!isset($this->_attributes_array[$_attribute_code])) {
            $_attribute = Mage::getModel('eav/config')->getAttribute('catalog_product', $_attribute_code);
            $this->_attributes_array[$_attribute_code] = $_attribute->getSource()->getAllOptions(false, false);
        }
        return $this->_attributes_array[$_attribute_code];
    }
    ...
    public function hasConfigurableAttribute($attribute_codes, $product_id) {
        ...
        if ($attribute_data) {
            ...
            if ($result) {
                ...
                /* use data from array , they are loaded only the first time and then reused */
                $attribute_options  = $this->_getAttributeOptions($attribute_data["attribute_code"]);
                //Mage::getModel('eav/config')->getAttribute('catalog_product', $attribute_data["attribute_code"]);
    ...
    public function hasConfigurableAttributeSimple($attribute_code, $product_id) {
        ...
        if ($data) {
            ...
            if ($result) {
                ...
                /* use data from array , they are loaded only the first time and then reused */
                $attribute_options  = $this->_getAttributeOptions($attribute_code);
                //Mage::getModel('eav/config')->getAttribute('catalog_product', $attribute_data["attribute_code"]);
                //print_r($attribute->getSource()->getAllOptions(false, false));
                foreach ($attribute_options as $option) {
                    $_labels[$option['value']] = $option['label'];
