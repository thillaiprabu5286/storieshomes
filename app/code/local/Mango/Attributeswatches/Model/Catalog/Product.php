...
class Mango_Attributeswatches_Model_Catalog_Product extends Mage_Catalog_Model_Product {
    ...
    public function getMediaGalleryImages() {
        if (!$this->hasData('media_gallery_images') && is_array($this->getMediaGallery('images'))) {
            ...
            /* new setting: exlude parent gallery and show only child products images.. */
            $_configurable_product = $this->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE;
            $_child_images = $_configurable_product && Mage::getStoreConfig('attributeswatches/settings/images') == "child";
            $_exclude_parent_images = $_child_images &&  Mage::getStoreConfig('attributeswatches/settings/exclude_parent_images');
            ...
            if(  !$_exclude_parent_images ){
                foreach ($this->getMediaGallery('images') as $image) {
                    /* skip if image is not associated with attributes, will hide in the template with css */
                    if ($image['disabled'] && (($this->getTypeId() == 'configurable' && !$image['associated_attributes']) || $this->getTypeId() != 'configurable'  )) {
                        continue;
                    }
                    $image['url'] = $this->getMediaConfig()->getMediaUrl($image['file']);
                    $image['id'] = isset($image['value_id']) ? $image['value_id'] : null;
                    $image['path'] = $this->getMediaConfig()->getMediaPath($image['file']);
                    if($_gallery_selectors){
                        /* json notation of config values */
                        $_config = trim(strtr( $image['associated_attributes'] , array( "attribute"=>"", " "=>"," , "-"=>":" )));
                        $image["config_value"] =  ( $_config ) ? "({" . $_config . "})" : ""  ;
                    }
                    $images->addItem(new Varien_Object($image));
                }
            }

            /* load images from child products in case the setting in the admin is set to true ( works only for configurable products )  */
            if ( $_child_images ) {
            ...
            /* at least one image in gallery */
            if(!$images->count() && $this->getImage() != 'no_selection' && $this->getImage() ){
                $image['url'] = $this->getMediaConfig()->getMediaUrl($this->getImage());
                $image['id'] = null;//isset($image['value_id']) ? $image['value_id'] : null;
                $image['path'] = $this->getMediaConfig()->getMediaPath($this->getImage());
                $images->addItem(new Varien_Object($image));
            }
