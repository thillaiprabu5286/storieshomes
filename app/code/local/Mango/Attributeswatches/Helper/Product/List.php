...
class Mango_Attributeswatches_Helper_Product_List extends Mage_Core_Helper_Abstract {
    ...
    /* used to store attributes and its options for faster rendering */
    protected $_resource_model = false;


    protected function _getResourceModel(){
        if(!$this->_resource_model){
            $this->_resource_model = Mage::getResourceModel('attributeswatches/attributes');
        }
    ...
    public function showAvailability() {
        if (!$this->_availability_info && $this->_product) {
            ...
            /* use resource model from function, so it can be reused... */
            $_resource_model = $this->_getResourceModel();
            $_availability_configuration = $_resource_model->hasConfigurableAttributeSimple($_availability_attribute, $_product->getId());
            $_availability_html = "";
                ...
                if (Mage::getStoreConfig("attributeswatches/layerednavigation/switchimages")) {
                    ...
                    foreach ($_vals as $_att) {
                        if (trim(Mage::app()->getRequest()->getParam($_att))){
                            $_values_to_check[$_att] = array(trim(Mage::app()->getRequest()->getParam($_att)));
                        }elseif(Mage::helper('core')->isModuleEnabled("Mango_Attributemapping")){
                            if(Mage::registry( "attribute_mapping_selected_values_" . $_att ))
                                $_values_to_check[$_att] = Mage::registry( "attribute_mapping_selected_values_" . $_att );
                        }
                    }
                }
                /* use resource model from function, so it can be reused... */
                $_resource_model = $this->_getResourceModel();
                $_swatches = $_resource_model->hasConfigurableAttribute($_configurable_attributes, $_product->getId());
//    $time_end = microtime(true);
//dividing with 60 will give the execution time in minutes other wise seconds
//    $execution_time = ($time_end - $time_start);
//execution time of the script
...
//            echo '<span style="display:none">after loading gallery ' . $execution_time . ' s.</span>';
                    ...
                    foreach ($_swatches as $product_id => $_option) {
                        ...
                        if (count($_values_to_check)) {
                            foreach ($_values_to_check as $_att => $_values) {
                                if ($_attribute_code == $_att && in_array($_option["value"], $_values)) { /* assuming multiple values can be selected in the layered navigation */
