...
class Mango_Attributeswatches_Block_Product_View_Type_Configurable extends Mage_Catalog_Block_Product_View_Type_Configurable {
    ...
    public function getJsonConfig() {
        ...
        foreach ($this->getAllowProducts() as $product) {
            ...
            if ($_attributes_to_reload) {
                foreach ($_attributes_to_reload as $attribute_code) {
                    ...
                    if ($attributeValue)
                        $_product_attributes[$productId][$attribute_code]['value'] = $attributeValue;
                    /* for select attributes */
                    $attributeText = $product->getAttributeText($attribute_code);
                    if ($attributeText)
                        $_product_attributes[$productId][$attribute_code]['text'] = $attributeText;
                }
            }
        }
        ...
        if ($preconfiguredFlag && !empty($defaultValues)) {
        ...
        } elseif (Mage::getStoreConfig("attributeswatches/settings/defaultselect")== "first") {
            ...
            $config['defaultValues'] = $defaultValues;
        }elseif (Mage::getStoreConfig("attributeswatches/settings/defaultselect")== "imageattribute") {
            /* set default for attributes based on 
             * combination of attributes associated to the 
             * selected image attribute */
            /* first, to load gallery image matching the chosen attribute
             * split values and retrieve attribute>value pairs */
            $config['defaultValues'] = Mage::getResourceModel("catalog/product_attribute_backend_media")->defaultOptions($currentProduct);
        }
        
        /* chained preselection - choose first available value of nextElement */
        $config['chainedPreselect'] = (bool)Mage::getStoreConfig("attributeswatches/settings/chained_preselect");
        
        /* use gallery images as option selectors */
        $config["gallerySelectors"] =  !(bool)Mage::getStoreConfig("attributeswatches/settings/switchgallery")  && (bool)Mage::getStoreConfig("attributeswatches/settings/gallery_swatches");
        
        /* use child product price as the final price of the configurable product */
        $_use_child_product_price = (int) Mage::getStoreConfig("attributeswatches/settings/use_child_product_price");
        $config["useChildProductPrice"] = $_use_child_product_price;
        if ($_use_child_product_price) {
            $config["priceUrl"] = Mage::getUrl('attributeswatches/price/getAjaxPrice');
            $config["simpleProductPrices"] = array();
        }
