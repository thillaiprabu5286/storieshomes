...
class Lotusbreath_OneStepCheckout_IndexController extends Lotusbreath_OneStepCheckout_Controller_Action
{
    ...
    public function reloadPartialAction()
    {

        $partials = $this->getRequest()->getParam('partial', false);

        $definedPartials = array(
            'shipping_partial',
            'payment_partial',
            'review_partial',
            'billing_partial',
            'shipping_address_partial',
        ...
        foreach ($partials as $partial) {
            if ($partial && in_array($partial, $definedPartials)) {
                if ($partial == 'review_partial')
                    $return['htmlUpdates'][$partial] = $this->_getReviewHtml();

                if ($partial == 'payment_partial')
                    $return['htmlUpdates'][$partial] = $this->_getPaymentHtml();

                if ($partial == 'shipping_partial')
                    $return['htmlUpdates'][$partial] = $this->_getShippingMehodHtml();

                $return['update_items'][] = $partial;
            }
        }
