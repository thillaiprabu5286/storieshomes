...
class TM_AjaxLayeredNavigation_Model_Mysql4_Price extends Mage_Catalog_Model_Resource_Eav_Mysql4_Layer_Filter_Price
{
    ...
    public function getAdvancedSearchLayer()
    {
    ...
    public function getMaxPrice($filter)
    {
        if ($this->isCatalogSearchPage()) {
            ...
            $layer = $this->getAdvancedSearchLayer();
            $collection = $layer->getProductCollection();
            ...
            $select->reset(Zend_Db_Select::GROUP); // clear group by for proper max execution
    ...
    public function getMinPrice($filter)
    {
        if ($this->isCatalogSearchPage()) {
            ...
            $select = $this->_getSelect($filter);
            $select->reset(Zend_Db_Select::GROUP); // clear group by for proper min execution
            $priceExpression = $this->_getFullPriceExpression($filter, $select);
