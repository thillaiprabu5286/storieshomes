...
abstract class Themevast_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = Themevast_Blog_Helper_Data::CATEGORY_URI_PARAM;

    protected static $_postUriParam = Themevast_Blog_Helper_Data::POST_URI_PARAM;

    protected static $_tagUriParam = Themevast_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    {
        if (self::$_helper->conf(Themevast_Blog_Helper_Data::XML_BOOKMARKS)) {
            return $this->setTemplate('themevast/blog/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('themevast/blog/line_tags.phtml')->setPost($post)->renderView();
        }
    }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == Themevast_Blog_Helper_Data::DEFAULT_ROOT;
    }
    ...
    protected function _prepareCollection()
    {
        if (!$this->getData('cached_collection')) {
            ...
            $collection = Mage::getModel('blog/blog')->getCollection()
                ->addPresentFilter()
                ->addEnableFilter(Themevast_Blog_Model_Status::STATUS_ENABLED)
                ->addStoreFilter()
                ->joinComments()
